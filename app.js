...
function clickHandler(e) {
  ...
  if(itemPurify(product.value.trim())) {
    if(array[1].includes(itemPurify(product.value.trim())) || charMatching.includes(itemPurify(product.value.trim()).toLowerCase())) {
      ...
      if(itemPurify(product.value.trim()) && QTYPurify(netWeight.value.trim()) && pricePurify(price.value.trim())) {
        num++
        array[0].push(num)
        ...
        array[4].push(QTYPurify(netWeight.value.trim()) * pricePurify(price.value.trim()))
...
function addingItemToList(array) {
  ...
  array.forEach((subArray, j) => {
    ...
    subArray.forEach((value, i) => {
      j === 3 || j === 4 ? element.innerHTML = value + " ৳" : element.innerHTML = value
      element.id = i
  ...
  icon_rack.addEventListener('click', (e) => deleteHandler(e, parentOfItemContainer, item_container, array))
}
...
// update serial number, item_container.id and grand total after delete an item
...
function presentingTime() {
  let array_of_months = ['জানুয়ারি', 'ফেব্রুয়ারি', 'মার্চ', 'এপ্রিল', 'মে', 'জুন', 'জুলাই', 'অগাস্ট', 'সেপ্টেম্বর', 'অক্টোবর', 'নভেম্বর', 'ডিসেম্বর']
  let array_of_day = ['রবিবার', 'সোমবার', 'মঙ্গলবার', 'বুধবার', 'বৃহস্পতিবার', 'শুক্রবার', 'শনিবার']
  let date = new Date()
  ...
  day_night_time.innerHTML = (`${array_of_day[day]}, ${english_digit_to_bangla(_date)}-${array_of_months[month]}-${english_digit_to_bangla(year)} (${day_night})`)
}
